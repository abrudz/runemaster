...
document.addEventListener('DOMContentLoaded', ()=>{
  fetch("r"+mr+"c"+mc+".json").then(d=>d.text()).then(d=>{
    j=JSON.parse(d)
  ...
  up   =()=>newR=Math.max(i.r-1,rMin)
  down =()=>newR=Math.min(i.r+1,rMax)
  left =()=>newC=Math.max(i.c-1,cMin)
  right=()=>newC=Math.min(i.c+1,cMax)
  addEventListener('keydown', e=>{
    newR=i.r;newC=i.c
    moved=0
    e.key=="Home"      ||e.key=="7"?up(left(moved=1)):0
    e.key=="ArrowUp"   ||e.key=="8"?up(moved=1):0
    e.key=="PageUp"    ||e.key=="9"?up(right(moved=1)):0
    e.key=="ArrowLeft" ||e.key=="4"?left(moved=1):0
    e.key=="Clear"     ||e.key=="5"||e.key==" "||e.key=="Enter"?moved=1:0
    e.key=="ArrowRight"||e.key=="6"?right(moved=1):0
    e.key=="End"       ||e.key=="1"?down(left(moved=1)):0
    e.key=="ArrowDown" ||e.key=="2"?down(moved=1):0
    e.key=="PageDown"  ||e.key=="3"?down(right(moved=1)):0
    if(moved&&!$$("dialog").some(e=>e.hasAttribute("open"))){
      ...
      if(t&&t.style.visibility!="hidden"){
        if(t.className=="l"){
          ...
          ch=j[t.id].req+j[t.id].add
          bi=$$("#belt b").map(e=>e.id)
          askb.innerHTML=ch.match(/../g,"").sort(_=>Math.random()-0.5).map(s=>s[1]==" "?`<button>${s[0]}</button>`:~bi.indexOf(s)?b(s):"").join("")
          $$("#askb>*").forEach(e=>e.setAttribute("onclick",
          "se=aski.selectionStart+1;"+
          "aski.value=aski.value.slice(0,aski.selectionStart)+this.innerText+aski.value.slice(aski.selectionEnd,aski.value.length);"+
          ...
          ch=c.id+j[c.id].req+j[c.id].add
          bi=$$("#belt b").map(e=>e.id)
          askb.innerHTML=ch.match(/../g,"").sort(_=>Math.random()-0.5).map(s=>s[1]==" "?`<button>${s[0]}</button>`:s==c.id||~bi.indexOf(s)?b(s):"").join("")
          $$("#askb>*").forEach(e=>e.setAttribute("onclick",
          "se=aski.selectionStart+1;"+
          "aski.value=aski.value.slice(0,aski.selectionStart)+this.innerText+aski.value.slice(aski.selectionEnd,aski.value.length);"+
          "aski.selectionStart=aski.selectionEnd=se;aski.focus()"))
